...
// Contract - договор с клиентом на мебель — деньги как decimal для точности копеек
type Contract struct {
	...
	TotalAmount   decimal.Decimal `json:"total_amount" gorm:"type:numeric(15,2)"`   // Общая сумма договора
	PrepaidAmount decimal.Decimal `json:"prepaid_amount" gorm:"type:numeric(15,2)"` // Сумма предоплаты
	PaidAmount    decimal.Decimal `json:"paid_amount" gorm:"type:numeric(15,2)"`    // Уже оплачено
	RemainAmount  decimal.Decimal `json:"remain_amount" gorm:"type:numeric(15,2)"`  // Остаток к оплате
	MarginPercent decimal.Decimal `json:"margin_percent" gorm:"type:numeric(5,2)"`  // Маржинальность в %
...
// Деньги — decimal (NUMERIC 12,2), на API отдаём float через InexactFloat64 (см. money.go).
